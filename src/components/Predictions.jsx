    ...
    <div className="space-y-8 fade-in">
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl">
          <div className="flex items-center space-x-2 text-orange-400 mb-4 font-bold tracking-wider uppercase text-sm">
            <Zap className="w-5 h-5" />
            <span>AI-Powered Insights</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-extrabold mb-4">High Probability Topics for 2026</h2>
      ...
      <div className="grid gap-6">
        {filteredPredictions.map((prediction, idx) => (
          <div key={idx} className="bg-white dark:bg-slate-800 rounded-2xl p-6 md:p-8 shadow-sm border border-slate-100 dark:border-slate-700 hover:shadow-md transition-all slide-up" style={{animationDelay: `${idx * 100}ms`}}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex-1">
                <h4 className="text-2xl font-bold mb-4">{prediction.topic}</h4>
                <div className="space-y-4">
                  <div className="flex items-start">
                    ...
                    <div>
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-300 block mb-1">Why this can come:</span>
                      <p className="text-slate-600 dark:text-slate-400">{prediction.reasoning}</p>
                    </div>
                  </div>
                  <div className="flex items-start">
                    ...
                    <div>
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-300 block mb-1">Previous Year Trends:</span>
                      <p className="text-slate-600 dark:text-slate-400">{prediction.trends}</p>
                    </div>
                  </div>
                </div>
              </div>
              
              <div className="md:w-48 shrink-0 flex flex-col items-center justify-center p-6 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                <div className={`text-4xl font-extrabold mb-2 ${getProbabilityColor(prediction.probability).split(' ')[0]}`}>
        ...
        {filteredPredictions.length === 0 && (
          <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700">
            <p className="text-slate-500">No predictions found for this filter.</p>
          </div>
        )}
      </div>
    </div>
