import fs from 'fs';
import { questions, studyPlan, aiPredictions, frequencyData, subjectFrequencies } from './src/data.js';
...
async function main() {
  ...
  console.log(`Found ${allCorrections.length} total corrections.`);
  ...
  let finalStr = `export const questions = ${JSON.stringify(questions, null, 2)};\n\n`;
  finalStr += `export const studyPlan = ${JSON.stringify(studyPlan, null, 2)};\n\n`;
  finalStr += `export const aiPredictions = ${JSON.stringify(aiPredictions, null, 2)};\n\n`;
  finalStr += `export const frequencyData = ${JSON.stringify(frequencyData, null, 2)};\n\n`;
  finalStr += `export const subjectFrequencies = ${JSON.stringify(subjectFrequencies, null, 2)};\n`;
